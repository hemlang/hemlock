...
# Find all test files (excluding compiler, parity, contracts, and formatter directories which have their own test runners)
TEST_FILES=$(find "$TEST_DIR" -name "*.hml" -not -path "*/compiler/*" -not -path "*/parity/*" -not -path "*/contracts/*" -not -path "*/formatter/*" | sort)
    ...
    # Skip vector tests if libusearch_c isn't installed
        ...
        if ! "$PROJECT_ROOT/hemlock" -e 'import "libusearch_c.so";' 2>/dev/null; then
            if [ "$category" != "$CURRENT_CATEGORY" ]; then
                ...
                echo -e "${BLUE}[$category]${NC}"
                echo -e "${YELLOW}⊘${NC} Skipping $category tests (libusearch_c not installed)"
                echo "  See stdlib/docs/vector.md for installation instructions"
                CURRENT_CATEGORY="$category"
            fi
            continue
        fi
