...
Value builtin_get_pid(Value *args, int num_args, ExecutionContext *ctx) {
    ...
    return val_i32((int32_t)hml_getpid());
}
...
done_warning:
    ...
    // Get exit code
    int status = hml_pclose(pipe);
    int exit_code = HML_WIFEXITED(status) ? HML_WEXITSTATUS(status) : -1;
    free(ccmd);
...
// Uses fork/execvp on POSIX, CreateProcess on Windows - preventing shell injection attacks
Value builtin_exec_argv(Value *args, int num_args, ExecutionContext *ctx) {
    ...
    // Use the process compat layer for spawning
    hml_process_t proc;
    if (hml_spawn_capture(argv[0], argv, &proc) != 0) {
        char error_msg[256];
        snprintf(error_msg, sizeof(error_msg), "exec_argv() failed to spawn process: %s", strerror(errno));
    ...
    char chunk[4096];
    ssize_t bytes_read;
    while ((bytes_read = hml_process_read_stdout(&proc, chunk, sizeof(chunk))) > 0) {
    ...
    int exit_code;
    hml_process_wait(&proc, &exit_code);
...
Value builtin_getppid(Value *args, int num_args, ExecutionContext *ctx) {
    ...
    return val_i32((int32_t)hml_getppid());
}
...
Value builtin_getuid(Value *args, int num_args, ExecutionContext *ctx) {
    ...
    return val_i32((int32_t)hml_getuid());
}
...
Value builtin_geteuid(Value *args, int num_args, ExecutionContext *ctx) {
    ...
    return val_i32((int32_t)hml_geteuid());
}
...
Value builtin_getgid(Value *args, int num_args, ExecutionContext *ctx) {
    ...
    return val_i32((int32_t)hml_getgid());
}
...
Value builtin_getegid(Value *args, int num_args, ExecutionContext *ctx) {
    ...
    return val_i32((int32_t)hml_getegid());
}
...
Value builtin_kill(Value *args, int num_args, ExecutionContext *ctx) {
    ...
    hml_pid_t pid = (hml_pid_t)value_to_int(args[0]);
    int sig = value_to_int(args[1]);

    if (hml_kill(pid, sig) != 0) {
        char error_msg[256];
        snprintf(error_msg, sizeof(error_msg), "kill(%d, %d) failed: %s", (int)pid, sig, strerror(errno));
        ctx->exception_state.exception_value = val_string(error_msg);
        ctx->exception_state.is_throwing = 1;
        return val_null();
    }
...
#ifdef HML_WINDOWS
    // Windows doesn't have fork() - return an error
    ctx->exception_state.exception_value = val_string("fork() is not supported on Windows");
    ctx->exception_state.is_throwing = 1;
    return val_null();
#else
    pid_t pid = fork();
...
#ifdef HML_WINDOWS
    // Windows doesn't have wait() - return an error
    ctx->exception_state.exception_value = val_string("wait() is not supported on Windows. Use exec_argv() instead.");
    ctx->exception_state.is_throwing = 1;
    return val_null();
#else
    int status;
...
#ifdef HML_WINDOWS
    // Windows doesn't have waitpid() - return an error
    ctx->exception_state.exception_value = val_string("waitpid() is not supported on Windows. Use exec_argv() instead.");
    ctx->exception_state.is_throwing = 1;
    return val_null();
#else
