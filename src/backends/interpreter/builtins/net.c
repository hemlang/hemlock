...
Value builtin_socket_create(Value *args, int num_args, ExecutionContext *ctx) {
    ...
    hml_platform_init();  // WSAStartup on Windows; no-op elsewhere
    int fd = socket(domain, type, protocol);
    if (fd < 0) {
        return throw_runtime_error(ctx, "Failed to create socket: %s", hml_sock_strerror(hml_sock_errno()));
    }
...
static Value socket_method_bind(SocketHandle *sock, Value *args, int num_args, ExecutionContext *ctx) {
    ...
    if (sock->domain == AF_INET) {
        ...
        if (bind(sock->fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
            return throw_runtime_error(ctx, "bind() failed: %s", hml_sock_strerror(hml_sock_errno()));
        }
...
static Value socket_method_listen(SocketHandle *sock, Value *args, int num_args, ExecutionContext *ctx) {
    ...
    if (listen(sock->fd, backlog) < 0) {
        return throw_runtime_error(ctx, "Failed to listen on socket: %s", hml_sock_strerror(hml_sock_errno()));
    }
...
static Value socket_method_accept(SocketHandle *sock, Value *args, int num_args, ExecutionContext *ctx) {
    ...
    if (client_fd < 0) {
        ...
        return throw_runtime_error(ctx, "Failed to accept connection: %s", hml_sock_strerror(hml_sock_errno()));
    }
    ...
    if (sock->domain == AF_UNIX) {
        ...
        if (connect(sock->fd, (struct sockaddr *)&addr_un, sizeof(addr_un)) < 0) {
            return throw_runtime_error(ctx, "Failed to connect to unix socket '%s': %s",
                    address, hml_sock_strerror(hml_sock_errno()));
        }
    } else {
        ...
        if (connect_result < 0) {
            return throw_runtime_error(ctx, "Failed to connect to %s:%d: %s",
                    address, port, hml_sock_strerror(hml_sock_errno()));
        }
    }
...
static Value socket_method_send(SocketHandle *sock, Value *args, int num_args, ExecutionContext *ctx) {
    ...
    if (sent < 0) {
        ...
        return throw_runtime_error(ctx, "Failed to send data: %s", hml_sock_strerror(hml_sock_errno()));
    }
...
static Value socket_method_recv(SocketHandle *sock, Value *args, int num_args, ExecutionContext *ctx) {
    ...
    if (received < 0) {
        ...
        free(data);
        return throw_runtime_error(ctx, "Failed to receive data: %s", hml_sock_strerror(hml_sock_errno()));
    }
...
static Value socket_method_recvfrom(SocketHandle *sock, Value *args, int num_args, ExecutionContext *ctx) {
    ...
    if (received < 0) {
        free(data);
        return throw_runtime_error(ctx, "Failed to recvfrom: %s", hml_sock_strerror(hml_sock_errno()));
    }
    ...
    result->fields[result->num_fields].name = strdup("data");
    if (!result->fields[result->num_fields].name) {
        free(buf->data);
        free(buf);
        object_free(result);
        return throw_runtime_error(ctx, "Memory allocation failed for field name");
    }
    result->fields[result->num_fields].value = data_val;
    result->num_fields++;

    // Add address field
    result->fields[result->num_fields].name = strdup("address");
    if (!result->fields[result->num_fields].name) {
        free(buf->data);
        free(buf);
        object_free(result);
    ...
    result->fields[result->num_fields].value = val_string(addr_str);
    result->num_fields++;

    // Add port field
    result->fields[result->num_fields].name = strdup("port");
    if (!result->fields[result->num_fields].name) {
        free(buf->data);
        free(buf);
        object_free(result);
    ...
    result->fields[result->num_fields].value = val_i32(src_port);
    result->num_fields++;
...
static Value socket_method_setsockopt(SocketHandle *sock, Value *args, int num_args, ExecutionContext *ctx) {
    ...
    // winsock declares optval as const char *
    if (setsockopt(sock->fd, level, option, (const void *)&value, sizeof(value)) < 0) {
        return throw_runtime_error(ctx, "Failed to set socket option: %s", hml_sock_strerror(hml_sock_errno()));
    }
...
static Value socket_method_set_timeout(SocketHandle *sock, Value *args, int num_args, ExecutionContext *ctx) {
...
#ifdef _WIN32
    // Windows expects the timeout as a DWORD in milliseconds
    ...
    if (setsockopt(sock->fd, SOL_SOCKET, SO_RCVTIMEO, (const void *)&timeout, sizeof(timeout)) < 0) {
        return throw_runtime_error(ctx, "Failed to set receive timeout: %s", hml_sock_strerror(hml_sock_errno()));
    }

    if (setsockopt(sock->fd, SOL_SOCKET, SO_SNDTIMEO, (const void *)&timeout, sizeof(timeout)) < 0) {
        return throw_runtime_error(ctx, "Failed to set send timeout: %s", hml_sock_strerror(hml_sock_errno()));
    }
...
#ifdef _WIN32
    u_long nonblocking_mode = enable ? 1 : 0;
    if (ioctlsocket(sock->fd, FIONBIO, &nonblocking_mode) != 0) {
        return throw_runtime_error(ctx, "Failed to set socket flags: %s", hml_sock_strerror(hml_sock_errno()));
    }
#else
    int flags = fcntl(sock->fd, F_GETFL, 0);
    if (flags < 0) {
        return throw_runtime_error(ctx, "Failed to get socket flags: %s", hml_sock_strerror(hml_sock_errno()));
    }
...
Value builtin_poll(Value *args, int num_args, ExecutionContext *ctx) {
    ...
    for (int i = 0; i < fds_arr->length; i++) {
        ...
        for (int j = 0; j < obj->num_fields; j++) {
            if (strcmp(obj->fields[j].name, "fd") == 0) {
                fd_val = obj->fields[j].value;
            } else if (strcmp(obj->fields[j].name, "events") == 0) {
                events_val = obj->fields[j].value;
            }
        }
    ...
    if (result < 0) {
        ...
        free(original_fds);
        char err_msg[256];
        snprintf(err_msg, sizeof(err_msg), "poll() failed: %s", hml_sock_strerror(hml_sock_errno()));
    ...
    for (int i = 0; i < fds_arr->length; i++) {
        if (pfds[i].revents != 0) {
            ...
            res_obj->fields[0].name = strdup("fd");
            if (!res_obj->fields[0].name) {
                object_free(res_obj);
                for (int j = 0; j < fds_arr->length; j++) {
                    value_release(original_fds[j]);
                }
            ...
            res_obj->fields[0].value = original_fds[i];
            value_retain(original_fds[i]);
            res_obj->num_fields = 1;

            res_obj->fields[1].name = strdup("revents");
            if (!res_obj->fields[1].name) {
                object_free(res_obj);
                for (int j = 0; j < fds_arr->length; j++) {
                    value_release(original_fds[j]);
                }
            ...
            res_obj->fields[1].value = val_i32(pfds[i].revents);
