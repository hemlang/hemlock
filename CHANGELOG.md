...
- **hemlockc: static lint pass.** A new advisory static-analysis pass flags well-typed, memory-safe code that is almost certainly a mistake: unreachable code (statements after `return`/`break`/`continue`/`throw`/`panic`, after an `if`/`else` whose arms all diverge, and after an infinite loop with no escaping `break` — the pass models break binding, so labeled and nested breaks are understood), dead branches (constant `if`/`while` conditions, redundant `else`), side-effect-free self-assignment (`x = x`, `obj.f = obj.f`, `a[i] = a[i]`), self-comparison (`x < x`, `x > x`, `x && x`, `x || x`; `==`/`!=`/`<=`/`>=` are deliberately exempt because `x != x` is the canonical NaN check), duplicate object-literal fields (the later value silently wins), duplicate literal `switch` cases (the second is unreachable), and literal modulo-by-zero (traps at runtime). Design mirrors the borrow checker: advisory by default, `--lint-error` promotes findings to fatal, `--lint-strict` adds unused-variable detection, `--no-lint` disables, and `--check` surfaces findings without compiling. The pass runs before the optimizer so it diagnoses the source as written, and only analyses the program's own top-level module so imported library code doesn't drown the user in warnings. Every diagnostic is conservative — a repo-wide sweep of 1200+ `.hml` files found zero false positives. New `tests/lint` suite (15 fixtures including negative false-positive guards), wired into `make test-lint` and `test-all`.
- **Borrow checker: full owned-resource coverage + interprocedural analysis.** The ownership checker now tracks the rest of Hemlock's owned resources beyond `alloc`/`buffer`/`open`, each with its correct release operation — `channel()` → `.close()`, `spawn()`/`spawn_with()` → `join()`/`detach()`/`await`, `ffi_open()` → `ffi_close()`, `mmap_open()`/`mmap_anon()` → `mmap_close()` — and flags release mismatches (e.g. `free()`ing a channel). A new lightweight interprocedural layer summarises how each top-level function treats its parameters (released on every path = "consumed", on some paths = "maybe-consumed", otherwise borrowed; summaries are order-independent so a callee may be defined after its callers): passing a live resource to a consuming parameter transfers ownership, so a later use is a use-after-free and a later free is a double free. Storing a resource into a container (object/array literal, property or index assignment) now counts as an escape, so handing ownership to a struct no longer trips strict-mode leak detection.
- **Compile-time rejection of unknown named arguments.** A named argument matching no parameter was silently bound positionally in compiled binaries while the interpreter threw "Unknown parameter name" — the same program behaved differently on the two backends. hemlockc now rejects it at compile time (`unknown named argument 'nam' to 'process'`), following the precedent of the too-many/too-few argument errors.

### Fixed

- **Warning-clean builds on gcc and clang.** Fixed the one remaining `-Wall -Wextra` warning (an unreachable `return` after the `noreturn` `hml_throw()` in the runtime's `hml_runtime_error_loc()`). Both backends, the runtime library, and the tools now build with zero warnings under gcc 13 and clang 18 — including with `-Wshadow -Wpointer-arith -Wundef -Wstrict-prototypes`.
- **Trigraph hazard in generated string literals.** A Hemlock string containing `??=`, `??(`, etc. — or *any* program using the `??=` operator, since hemlockc embeds the full source text via `hml_set_source()` for runtime-error snippets — emitted the sequence verbatim inside a C string literal. Every compile printed `-Wtrigraphs` warnings from the internal gcc invocation, and compiling `--emit-c` output with `-std=c11` (trigraphs enabled) silently rewrites the literal (`??=` → `#`). The codegen string escaper now breaks the sequence (`?\?=`).
- **hemlockc's generated C is now `-Wall -Wextra`-clean on gcc and clang** (409 warnings → 0 across the emitted C of all 312 parity tests) — relevant to anyone feeding `--emit-c` output into their own build. Codegen suppresses structurally-may-be-unused constructs: `(void)` casts for the closure-env parameter and never-read Hemlock parameters, `HML_UNUSED_LABEL` (label `__attribute__((unused))`, defined in `hemlock_runtime.h`) on break/continue/match/switch/tail-call labels, and `HML_MAYBE_UNUSED` on import-alias bindings and unboxed locals. The `@optimize` annotation now emits through `HML_ATTR_OPTIMIZE`, which expands empty under clang (no per-function optimize attribute there; it warned `-Wunknown-attributes`). An out-of-int32-range literal loop bound now compares through a `const int64_t` local (folds identically at `-O3`; the bare literal tripped `-Wtype-limits`).
